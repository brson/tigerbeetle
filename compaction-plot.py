...
def drop_high_space(data):
    seeds, configs = build_views(data)
    blocks_active_summed = []

    for config in configs.values():
        blocks_active_total = 0
        for row in config:
            blocks_active_total += row[3]
        blocks_active_summed += [(blocks_active_total, config)]

    blocks_active_summed.sort(
        key = lambda x: x[0],
    )

    blocks_active_summed = blocks_active_summed[:math.floor(len(blocks_active_summed)/2)]

    new = []
    for _, row in blocks_active_summed:
        new += row
...
reduced_data2 = drop_high_space(reduced_data)
