...
# Configure Wine to disable automatic backtraces
...
build-tb-client-win:
    bash -c 'unset JAVA_HOME; {{ZIG}} build clients:rust -Dtarget=x86_64-windows -Drelease=false 2>&1 | tee -a build-tb-client-win.log; exit ${PIPESTATUS[0]}'
...
# Find the newest exe matching a glob (avoids stale binaries breaking glob expansion)
[private]
newest-exe pattern:
    #!/usr/bin/env bash
    ls -t {{pattern}} 2>/dev/null | head -1

# Run Rust tests under Wine (integration tests with external server on port 3001)
test-rust-win: build-rust-test-win
    #!/usr/bin/env bash
    set -euo pipefail
    # Run unit tests (don't need server)
    unit_exe=$(ls -t src/clients/rust/target/x86_64-pc-windows-gnu/debug/deps/tigerbeetle-*.exe | head -1)
    #{{wine64}} "$unit_exe"
    # Run integration tests using external server on port 3001
    # Start server first with: ./tigerbeetle start --addresses=127.0.0.1:3001 --development <dbfile>
    test_exe=$(ls -t src/clients/rust/target/x86_64-pc-windows-gnu/debug/deps/tests-*.exe | head -1)
    RUST_BACKTRACE=1 TIGERBEETLE_SERVER_PORT=3001 {{wine64}} "$test_exe" --test-threads=1 multithread
...
test-rust-win-gdb: build-rust-test-win
    ...
    RUST_BACKTRACE=1 TIGERBEETLE_SERVER_PORT=3001 gdb -batch \
        ...
        -ex 'info registers' \
        -ex 'quit' \
        --args {{wine64}} "$test_exe" --test-threads=1 multithread
...
test-rust-win-rr-record: build-rust-test-win
    ...
    test_exe=$(ls -t src/clients/rust/target/x86_64-pc-windows-gnu/debug/deps/tests-*.exe | head -1)
    RUST_BACKTRACE=1 TIGERBEETLE_SERVER_PORT=3001 rr record {{wine64}} "$test_exe" --test-threads=1 multithread
