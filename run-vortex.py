...
def build_driver(driver_name: str):
    ...
    if driver_name == "python":
        try:
            run_command(
                [ZIG_CMD, "build", "clients:python", "-Drelease"],
            )
        except subprocess.CalledProcessError as e:
            print(f"❌ Build failed: {e}")
            sys.exit(e.returncode)
    ...
    if driver_name == "node":
        try:
            run_command(
                [ZIG_CMD, "build", "clients:node", "-Drelease"],
            )
            run_command(
                ["npm", "install"],
                cwd="./src/clients/node",
            )
            run_command(
                ["npm", "install"],
                cwd="./src/testing/vortex/node_driver",
            )
            run_command(
                ["npm", "run", "build"],
                cwd="./src/testing/vortex/node_driver",
...
def make_driver_args(driver_name: str) -> list[str]:
    if driver_name == "java":
        ...
        return [f"--driver-command={command}"]
    elif driver_name == "node":
        command = "node src/testing/vortex/node_driver/dist/main.js"
        return [f"--driver-command={command}"]
    else:
        return []
