    ...
    return struct {
        ...
        fn cancel_request_inflight(self: *Context) void {
            ...
            if (self.client.request_inflight) |*inflight| {
                // With connection eviction, we can be evicted while registering.
                // Registration requests don't have user packets - the user_data is a
                // Context pointer, not a UserData struct. Skip packet cancellation for these.
        ...
        fn signal_notify_callback(signal: *Signal) void {
            ...
            // Also return early if evicted (which can happen during registration
            // due to connection pool eviction).
            if (self.batch_size_limit == null) {
                if (self.eviction_reason != null) {
                    // Evicted during registration. Drain and cancel submitted packets
                    // so user callbacks are invoked and their futures don't deadlock.
                    while (true) {
