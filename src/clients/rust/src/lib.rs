...
impl Client {
    ...
    /// [`CreateAccountResult::Exists`], described below).
    /// This behavior reflects optimizations in the underlying protocol.
    /// This client will never return a `CreateAccountResult::Ok`; that variant is defined
    /// in case it is useful for clients to materialize omitted request results.
    ///
    /// To relate a `CreateAccountsResult` to its input event, the [`CreateAccountsResult::index`] field
    ...
    /// In other words, this function does not return results for successful
    /// events, only unsuccessful events (though note the case of
    /// [`CreateTransferResult::Exists`], described below).
    /// This behavior reflects optimizations in the underlying protocol.
    /// This client will never return a `CreateTransferResult::Ok`; that variant is defined
    /// in case it is useful for clients to materialize omitted request results.
    ///
