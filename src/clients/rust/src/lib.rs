...
//! Rust async runtime. Instead it contains it's own off-thread event loop,
...
//!
//! If there are too many results for a query, then you can page
//! through them by setting `timestamp_min` and `timestamp_max`.
//! The optimal paging algorithm in this case is unclear.
//!
//! In TigerBeetle's standard build-time configuration **the maximum number of
...
//! of these is application-specific, and any scheme that guarentees unique IDs
...
//! implementation in the [`id`] fuction.
//!
...
impl Client {
    ...
    /// TigerBeetle's standard built-time configuration, the maximum batch size
    /// is 8189.
    ///
    /// # Errors
    ///
    ...
    /// # Protocol reference
    ///
    /// [`create_transfers`](https://docs.tigerbeetle.com/reference/requests/lookup_transfers).
    ...
    ///
    /// Note that it is not required for correctness to call this method &emdash;
    /// `Client`'s destructor will correctly shut down the client, though
    /// without providing the ability to wait for shutdown.
    ///
    /// This method is provided for those that wish to to ensure that the client's
