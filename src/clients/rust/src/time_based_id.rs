...
///   sequentially starting at the unix epoch.
//
// References:
//
...
pub fn id() -> u128 {
    GLOBAL_GENERATOR
        .lock()
        .expect("global tbid generator")
        .next()
}

static GLOBAL_GENERATOR: LazyLock<Mutex<TbidGenerator>> =
    LazyLock::new(|| Mutex::new(TbidGenerator::new()));
...
impl TbidGenerator {
    ...
    fn next_from_system_time(&mut self, now: SystemTime) -> u128 {
        *self = self.next_state_from_system_time(now);
        ...
        let id = self.ms_since_epoch << 80 | self.random;
