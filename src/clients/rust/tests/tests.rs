...
fn result_correlation() -> anyhow::Result<()> {
    ...
    block_on(async {
        ...
        assert_eq!(result[0].result, tb::CreateAccountResult::LedgerMustNotBeZero);
        assert_eq!(result[1].index, 3);
        assert_eq!(result[1].result, tb::CreateAccountResult::CodeMustNotBeZero);
...
// A runnable version of the `create_accounts` example.
// If you update this please update that.
#[test]
fn example_create_accounts() -> Result<(), Box<dyn std::error::Error>> {
    ...
    block_on(async {
        ...
        let results_merged: Vec<_> = merge_create_accounts_results(&accounts, results_actual).collect();
        let results_expected: Vec<(&tb::Account, tb::CreateAccountResult)> = accounts.iter().zip(merged_results_expected).map(|(account, create_account_result)| {
...
// A runnable version of the `create_transfers` example.
// If you update this please update that.
#[test]
...
// A runnable version of the `lookup_accounts` example.
// If you update this please update that.
#[test]
fn example_lookup_accounts() -> Result<(), Box<dyn std::error::Error>> {
    ...
    block_on(async {
        ...
        let results_actual: Vec<_> = results_actual.into_iter().map(|account| {
            tb::Account {
        ...
        let merge_actual: Vec<_> = merge_lookup_accounts_results(accounts_lookup, results_actual).collect();
...
fn example_lookup_transfers() -> Result<(), Box<dyn std::error::Error>> {
    ...
    block_on(async {
        ...
        let results_actual: Vec<_> = results_actual.into_iter().map(|transfer| {
            tb::Transfer {
                timestamp: Default::default(),
                .. transfer
            }
        ...
        let merge_actual: Vec<_> = merge_lookup_transfers_results(transfers_lookup, results_actual).collect();
        assert_eq!(merge_expected, &merge_actual[..]);
