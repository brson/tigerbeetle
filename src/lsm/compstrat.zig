...
pub const LevelStrategy = enum {
    AllSame,
    Small2NoLookAlwaysMoveLargeLookMove,
    Small2NoLookAlwaysMoveLargeSparse10,
};
...
pub const CompStrat = struct {
    ...
    pub fn init(allocator: mem.Allocator) !*Self {
        ...
        var comp_level = LevelStrategy.AllSame;
        ...
        const comp_level_str = env_map.get(comp_level_var) orelse comp_level_var_default;
        if (mem.eql(u8, comp_level_str, "SAME")) {
            comp_level = LevelStrategy.AllSame;
        } else if (mem.eql(u8, comp_level_str, "S2NO-LYES")) {
            comp_level = LevelStrategy.Small2NoLookAlwaysMoveLargeLookMove;
        } else if (mem.eql(u8, comp_level_str, "S2NO-LSPARSE10")) {
            comp_level = LevelStrategy.Small2NoLookAlwaysMoveLargeSparse10;
        } else {
            @panic("bad COMP_LEVEL");
        }
    ...
    pub fn deinit(self: *Self, allocator: mem.Allocator) void {
        // fixme leak - manifest_level test fails if this is enabled
        //self.env_map.deinit();
        allocator.destroy(self);
    }

    pub fn log_final_stats(self: *const Self, compaction_stats: *const CompactionStats) void {
    ...
    pub fn pick_table_candidate(
        ...
        const old_mid_value_vicinity = @abs(@as(i64, @intCast(old_value_count)) -
            @as(i64, @intCast(old_table_mid_values)));
        const new_mid_value_vicinity = @abs(@as(i64, @intCast(new_value_count)) -
            @as(i64, @intCast(new_table_mid_values)));

        const new_best = switch (self.select) {
            Selection.LeastTables => (new_table_count < old_table_count),
            Selection.MostTables => (new_table_count > old_table_count),
            Selection.LeastValues => (new_value_count < old_value_count),
            Selection.MostValues => (new_value_count > old_value_count),
            Selection.MidValues => (new_mid_value_vicinity < old_mid_value_vicinity),
            Selection.HighTableValueRatio => (new_ratio > old_ratio),
            Selection.LowTableValueRatio => (new_ratio < old_ratio),
    ...
    pub fn with_lookaround_policy(self: *const Self, level_a: u8) ?LookaroundPolicy {
        return switch (self.level) {
            LevelStrategy.AllSame => self.with_lookaround_policy_(),
            LevelStrategy.Small2NoLookAlwaysMoveLargeLookMove =>
                if (level_a < 2) null else self.with_lookaround_policy_(),
            LevelStrategy.Small2NoLookAlwaysMoveLargeSparse10 => @panic("todo"),
        };
    }

    fn with_lookaround_policy_(self: *const Self) ?LookaroundPolicy {
    ...
    pub fn post_lookaround_policy(self: *const Self, level_a: u8) ?LookaroundPolicy {
        return switch (self.level) {
            LevelStrategy.AllSame => self.post_lookaround_policy_(),
            LevelStrategy.Small2NoLookAlwaysMoveLargeLookMove =>
                if (level_a < 2) null else self.post_lookaround_policy_(),
            LevelStrategy.Small2NoLookAlwaysMoveLargeSparse10 => @panic("todo"),
        };
    }

    fn post_lookaround_policy_(self: *const Self) ?LookaroundPolicy {
        return switch (self.look) {
            Lookaround.None => null,
