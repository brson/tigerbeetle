...
pub fn ManifestType(comptime Table: type, comptime Storage: type) type {
    ...
    return struct {
        ...
        pub fn find_table_coalesce_window(
            ...
            if (!std.mem.eql(u8, manifest.config.name, "tigerbeetle.Transfer")) {
                return null;
            }

            std.log.info("maybe coalescing {s}, level {}, {} tables", .{
...

            {
                ...
                while (it.next()) |table| {
                    std.log.info("value_count {}", .{ table.value_count });
                }
            }
            ...
            std.log.info("num tables {}, ns {}, found {}", .{
                num_tables, ns, found,
            });
        ...
        fn find_table_coalesce_window_slice(
            ...
            for (tables) |table| {
                ...
                if (can_free_table) {
                    std.log.info("can free table {} >= {}, from {} tables", .{
